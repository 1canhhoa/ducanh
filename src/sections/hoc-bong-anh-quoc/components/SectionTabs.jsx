...
const SectionTabs = ({items = [], className = ''}) => {
  ...
  return (
    <section className={className}>
      <nav className='flex items-center w-full no-scrollbar xmd:pl-[0.75rem] xmd:overflow-x-auto xmd:p-[0.5rem] p-[0.25rem] xmd:border-none border border-primary-20 bg-white rounded-xl'>
        {Array.from({length: 10}).map((tabItem, index) => {
          return (
            <div
              key={`${tabItem}` + index}
              onClick={() => handleActiveIndexChange(index)}
              className={clsx(
                `ml-[0.5rem] first:ml-0 rounded-lg xmd:p-[0.75rem] p-[0.625rem_1.25rem] h-[2.625rem] 
