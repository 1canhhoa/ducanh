...
const HocSinhTieuBieu = ({isMobile, dataHocSinhTieuBieu}) => {
  ...
  return (
    <div className='w-full xmd:mt-[3rem] mt-[6rem] relative'>
      <Image
        loading='lazy'
        alt='background học sinh tieu biểu web'
        src='/images/homepage/bg-hocsinh.png'
        className='w-full h-[49.25rem] z-[9] shrink-0 xmd:hidden'
        width={1600}
        height={790}
      />
      <Image
        loading='lazy'
        alt='background học sinh tieu biểu mobi'
        src='/images/homepage/bg-hocsinh-mobi.png'
        className='w-full h-[50.65886rem] z-[9] shrink-0 md:hidden'
        width={380}
        height={700}
      />
      <div className='absolute  xmd:hidden top-16 left-16'>
        <div className='flex flex-col items-start space-y-[0.5rem]'>
          <div
            className={
              'text-primary-10 text-xl not-italic font-bold leading-[120%] tracking-[-0.00313rem] uppercase'
            }
          >
      ...
      >
        {dataHocSinhTieuBieu?.map((_, i) => (
          <div
            key={i}
            className={
              (active === i ? ' bg-[#FFF] ' : ' opacity-40 bg-[#FFF]') +
      ...
      {dataHocSinhTieuBieu?.map((c, index) => (
        <div
          key={index}
          className={
            'inline-flex absolute xmd:flex-col xmd:space-y-[1rem] xmd:top-[2.81rem] top-[22%] xmd:left-[1.25rem] md:left-[48%] md:-translate-x-1/2  items-start  md:p-[1.8125rem] duration-300 ease-linear ' +
            (index !== active ? 'opacity-0' : ' opacity-100')
          }
        >
          <Image
            loading='lazy'
            alt='dấu quotes trích dẫn'
            src='/images/homepage/quotes.png'
            className='w-[2.5rem] h-[1.75rem] shrink-0'
            width={40}
            height={28}
          />
          <div className='xmd:hidden w-5 z-50 h-[16rem]'></div>
      ...
      <div
        style={{
          top: !isMobile
            ? `calc(11.8rem + 1.8125rem + 1rem + ${divHeight}px)`
            : `calc(5.8rem + 1.8125rem + 1rem + ${divHeight}px)`,
        }}
      ...
      >
        <button className=' relative flex justify-center items-center xmd:h-[2.6rem] h-[3rem] xmd:py-[0.9375rem] py-[0.75rem] xmd:px-[2.5rem] px-[1.875rem] rounded-[0.625rem]  bg-linear-l5  '>
          <div className='flex justify-center items-center absolute xmd: xmd:h-[92%] top-1/2  -translate-y-1/2 left-1/2 -translate-x-1/2 h-[2.7rem] w-[97%] rounded-[0.525rem] bg-white   '></div>
          <div
        ...
        <button
          className='flex justify-center items-center xmd:h-[2.5rem] h-[3rem] xmd:py-[0.9375rem] py-[0.75rem] px-[1.875rem]
                rounded-[0.625rem] xmd:border-[1px] xmd:border-primary-10 border-[2px]'
        >
          <div
            className='text-white w-max text-center xmd:normal-case uppercase
      ...
      <div className=' slider-hocsinh'>
        {dataHocSinhTieuBieu?.map((d, i) => (
          <Image
            ...
            height={730}
            alt={`ảnh học sinh ${i}`}
            className='item xmd:!bottom-[-5.1rem] !bottom-[-3.1rem]'
