...
const NavMobi = ({ openNavMobile, data_header, setOpenNavMobile }) => {
  ...
  return (
    ...
    <div className={"block overflow-y-auto no-scrollbar h-screen w-screen fixed top-0 bottom-0 left-0 z-[999999]] bg-white lg:hidden ease-in-out transition-all duration-300"
      ...
      <div className='flex w-full bg-white h-[4.09956rem] justify-between items-start p-[0.87848rem] border-b-2 border-b-white border-solid'>
        <Link
          ...
          href='/'
        >
          <Image onClick={handleClickItem} priority alt="ảnh logo du học đức anh" src={data_header?.logo} width={235} height={69}
            className="xmd:w-[8.19912rem] xmd:h-[2.2694rem] xlg:w-[19rem] xlg:h-[6.9375rem]   w-[10.65513rem] h-[3.125rem]" />
        </Link>
        <div onClick={() => setOpenNavMobile(false)} className="flex justify-start items-center gap-[0.87848rem] p-[0.43924rem]">
          <svg className='size-[1.46413rem]' xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M15 15L10 10M10 10L5 5M10 10L15 5M10 10L5 15" stroke="#A9A9A9" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
          </svg>
        </div>
                ...
                <div onClick={() => handleClick(i)} className={'flex group rounded-full justify-between items-center '}>
                  <li key={i} className='flex h-5 space-x-[0.75rem] items-start self-stretch'>
                    ...
                    <div className="flex w-5 h-5 justify-center items-center">
                      <svg className={`${accordionOpen && active === i ? 'rotate-180' : 'rotate-360'} duration-200 ease-linear w-[0.525rem] h-[0.525rem]`} xmlns="http://www.w3.org/2000/svg" width="8" height="6" viewBox="0 0 8 6" fill="none">
