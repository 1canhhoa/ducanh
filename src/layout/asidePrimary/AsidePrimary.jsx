...
const AsidePrimary = async () => {
  ...
  const dataSibar = data?.acf["side-bar"]
  return (
        ...
        {dataSibar?.map((d, i) => (
          <li key={i} >
            <div className=" z-[2] cursor-pointer
              ...
              group w-[21.8125rem] h-[3.2rem] relative  px-[1.5rem] py-[0.5]rem]">
              ...
              <Image priority alt={`ảnh background lá cờ ${d?.title} `} src={d?.bg} width={360} height={60}
                className={`${i === dataSibar?.length - 1 ? "rounded-br-xl rounded-bl-xl" : ""} absolute top-0 z-[-2] w-full h-full group-hover:opacity-100  custom-transition opacity-0 left-0 right-0 bottom-0`} />

              <div className=' button1 absolute  group-hover:opacity-100 duration-700 opacity-0 flex-1 z-[1]  text-white uppercase '>
                {d?.title}</div>

            </div>
            {i !== dataSibar?.length - 1 && <div className=" w-[18.125rem] my-[0.033rem] h-[0.09025rem] mx-auto shrink-0 bg-primary-60 opacity-10"></div >}
          </li>
        ))}
      </ul>
    </div>
